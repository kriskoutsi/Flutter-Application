...
class GenreWidget extends StatefulWidget {
  final GenreItem curGenre;
  const GenreWidget({Key? key, required this.curGenre}) : super(key: key);
  @override
  State<StatefulWidget> createState() => _GenreWidget();
}

class _GenreWidget extends State<GenreWidget> {
  //genList contains the upcoming events for this genre
  final genList = <EventItem>{};
  ...
  Widget build(BuildContext context) {
    for (var i = 0; i < dummyConcerts1.length; i++) {
      if (dummyConcerts1[i].genre == widget.curGenre.genre) {
        genList.add(dummyConcerts1[i]);
      }
    }
    for (var i = 0; i < dummyArtists.length; i++) {
      if (dummyArtists[i].genre == widget.curGenre.genre) {
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) =>
                  buildCard(event: genList.elementAt(index)),
            ),
          ),
          const SizedBox(
            height: 40,
          ...
          ClipRRect(
              ...
              child: Container(
                  ...
                  child: TextButton(
                    onPressed: () {
                      setState(() {
                        if (savedGenres.contains(widget.curGenre)) {
                          savedGenres.remove(widget.curGenre);
                        } else {
                          savedGenres.add(widget.curGenre);
                          for (var i = 0;
                              i < dummyEventNotifications.length;
                              i++) {
                            if (widget.curGenre.genre ==
                    ...
                    child: savedGenres.contains(widget.curGenre)
                        ? const Text(
                            'FOLLOWING',
                            style: TextStyle(
                              color: Colors.white,
